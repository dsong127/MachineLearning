...
def general_search(problem, queueing_func):
    ...
    while not nodes.empty():
        ...
        if (entire_node[2] or entire_node[1]):
            print
            "The best state to expand with a g(n) = %d and h(n) = %d is.." % (entire_node[2], entire_node[1]),
        print_board(node)
        if problem.goal_test(node):
            print
            "Goal State"
            print
            "To solve this problem the search algorithm expanded a total of %d nodes." % problem.get_level()
            print
            "The maximum number of nodes in the queue at any one time was %d." % nodes.get_max_elements()
            print
            "The depth of the goal node was %d" % entire_node[2]
            return node
        print
...
if __name__ == "__main__":
    print
    "Welcome to the awesome %d-puzzle solver." % PUZZLE_TYPE
    print
    ...
    if choice == 1:
        ...
        print
        "Enter elements for %d Puzzle." % PUZZLE_TYPE
        print
        "NOTE: Use \"x\" for blank.\n"
    ...
    t2 = time.time()
    print
    "The algorithm took " + str((t2 - t1) * 1000) + " ms of time."